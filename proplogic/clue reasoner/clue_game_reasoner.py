...
class ClueGameReasoner:
    ...
    def __init__(self, player_order, card_nums = None):
        ...
        for c in CARDS:
            for i, p1 in enumerate(POSSIBLE_CARD_LOCATIONS):
                for p2 in POSSIBLE_CARD_LOCATIONS[i + 1:]:
                    clauses.append(f"~{c}_{p1} ~{c}_{p2}")
        ...
        for category in [SUSPECTS, WEAPONS, ROOMS]:
            for i, c1 in enumerate(category):
                for c2 in category[i + 1:]:
                    clauses.append(f"~{c1}_{CASE_FILE} ~{c2}_{CASE_FILE}")
    ...
    def add_hand(self, player_name, hand_cards):
        ...
        for card in hand_cards:
            self.KB.add_clause(f"{card}_{player_name}")
        # TO BE IMPLEMENTED AS AN EXERCISE

    def suggest(self, suggester, c1, c2, c3, refuter, cardshown):
        '''Add information about a given suggestion to the KB'''
        for i in range(len(self.players)):
            if self.players[i] == suggester:
                # print(self.players[i], "is a suggester")
                for j in range(i+1,i + 1+ len(self.players)):
                    index = j % len(self.players)
                    if refuter!=self.players[index] and refuter is not None:
                        self.KB.add_clause(f"~{c1}_{self.players[index]}")
                        self.KB.add_clause(f"~{c2}_{self.players[index]}")
                        self.KB.add_clause(f"~{c3}_{self.players[index]}")
                        # print(self.players[index], "is not a refuter")
                        # print()
                    elif refuter==self.players[index] and refuter is not None:
                        # print(self.players[index], "is a refuter")
                        if cardshown is not None:
                            self.KB.add_clause(f"{cardshown}_{self.players[index]}")
                        elif cardshown is None:
                            self.KB.add_clause(f"{c1}_{self.players[index]} {c2}_{self.players[index]} {c3}_{self.players[index]}")
                        break
                    elif refuter is None:
                        for p in self.players:
                            if p!=self.players[i]:
                                self.KB.add_clause(f"~{c1}_{p}")
                                self.KB.add_clause(f"~{c2}_{p}")
                                self.KB.add_clause(f"~{c3}_{p}")
    def accuse(self, accuser, c1, c2, c3, iscorrect):
        '''Add information about a given accusation to the KB'''
        if iscorrect:
            self.KB.add_clause(f"{c1}_{CASE_FILE} {c2}_{CASE_FILE} {c3}_{CASE_FILE}")
        else:
            # If the accusation is incorrect, update the KB with the information
            self.KB.add_clause(f"~{c1}_{CASE_FILE}")
            self.KB.add_clause(f"~{c2}_{CASE_FILE}")
            self.KB.add_clause(f"~{c3}_{CASE_FILE}")
        # TO BE IMPLEMENTED AS AN EXERCISE
...
if __name__ == '__main__':
    print("OUPUT OF GAME 1")
    play_clue_game1()
    print("OUPUT OF GAME 2")
    play_clue_game2()
    print("OUPUT OF GAME 3")
    play_clue_game3()